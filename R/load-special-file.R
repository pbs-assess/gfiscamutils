#' Load the specially-formatted age fit, age residuals, or selectivity
#' estimates file. Loading uses parallelism by gear
#'
#' @param fn The filename
#' @param gear_names A vector of gear names to use in the resulting data frame.
#' If `NULL`, the gear numbers set in the iscam data file will be used
...
load_special <- function(fn,
  ...
  if(length(lst) == length(gear_names)){
    ...
    warning("`gear_names` was not the same length as what was reported in the ",
            "mcmc output file ", fn, ". Using gear numbers as found in the ",
            "data file instead.")
  }
